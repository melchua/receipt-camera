...
export default class ReceiptFormModal extends React.Component {
  ...
  componentWillMount() {
    ...
    fetch(LOCALURL + '/projects', {
      ...
      .then(fetch(LOCALURL + '/categories', {
          method: 'GET',
          headers: {
            'Content-Type': 'application/json',
          }
        ...
        .then((categories) => {
          ...
          categories.forEach(function (category) {
            catObj[category.cat_name] = category.id
          })
          this.setState({
            catObj: catObj
          })
  ...
  render() {
    return ( 
      <FormValidator total = {this.state.total} date = {this.state.date} projects = {this.state.projects} isValid = {this._isValid} projectObj = {this.state.projectObj} catObj = {this.state.catObj} />
    );
  }
}